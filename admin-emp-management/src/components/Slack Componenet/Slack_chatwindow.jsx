  ...
  return (
    <div className="relative">
      ...
      <p className={`whitespace-pre-wrap break-words text-[15px] leading-relaxed ${
        ...
        {displayedText}
        {!isExpanded && (
          <span className={`${isMe ? 'text-blue-200/60' : 'text-gray-500/60'}`}>...</span>
        )}
      </p>
      
      {/* Gradient Fade Effect (only when collapsed) */}
      {!isExpanded && (
        <div className={`absolute bottom-0 right-0 h-6 w-20 pointer-events-none ${
          isMe 
      ...
      <div className="px-6 py-4 border-b border-gray-200 bg-white/95 backdrop-blur-sm shadow-sm">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="font-bold text-gray-800 text-lg">{selectedUser.name}</h2>
              <div className="flex items-center space-x-2">
                <p className="text-sm text-gray-500">
                  {onlineUsers && onlineUsers.includes(selectedUser._id) 
                    ? <span className="flex items-center"><span className="w-2 h-2 bg-green-500 rounded-full mr-2"></span> Online now</span>
                    : 'Last seen recently'
                  }
                </p>
        ...
        <div className="max-w-4xl mx-auto space-y-3">
          {messages.map((m) => {
            ...
            return (
              ...
              >
                <div className={`max-w-[70%] lg:max-w-[60%] ${isMe ? 'ml-auto' : ''}`}>
                  {/* Message Bubble */}
            ...
            <div className="mb-1">
              <MessageText text={m.text} isMe={isMe} />
                    ...
                    {/* Files */}
                    {Array.isArray(m.files) && m.files.map((f, idx) => {
                      const icon = getFileIcon(f.type, f.name);
                      const src = `http://localhost:5000${f.url}`;
                    ...
                    <div className={`flex items-center justify-end gap-2 mt-2 ${
                      ...
                      {isMe && (
                        <div className="flex items-center">
                          ...
                          {m.deliveredAt && !m.seenAt && <CheckCheck className="w-3.5 h-3.5" />}
                          {m.seenAt && <CheckCheck className="w-3.5 h-3.5 text-[#03f4fc]" />}
                        </div>
                      )}
                    </div>
                  </div>
