...
export default function DateRangePicker({ fromDate, toDate, onChange }) {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (pickerRef.current && !pickerRef.current.contains(event.target)) {
        setOpen(false);
        // Reset local state to prop values if dropdown closed without apply
  ...
  const applyPreset = (type) => {
    ...
    const newFrom = start.format("YYYY-MM-DD");
    const newTo = end.format("YYYY-MM-DD");

    setLocalFrom(newFrom);
    setLocalTo(newTo);
    onChange({ fromDate: newFrom, toDate: newTo }); // Apply immediately for presets
    setOpen(false);
  };

  const handleApply = () => {
    onChange({ fromDate: localFrom, toDate: localTo });
    setOpen(false);
  };
